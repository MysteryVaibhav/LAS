...
class DataLoader:
    def __init__(self, params):
        ...
        if params.mode == 0:
            ...
            self.train_1, _, self.train_transcript_1, _ = train_test_split(self.train_1, self.train_transcript_1, test_size=0.05, random_state=100)
            ...
            self.train_2, _, _, _ = train_test_split(self.train_2, self.train_transcript_2, test_size=0.05, random_state=100)

            self.train_3 = self.expand(np.load(params.data_dir + 'feats40dim_train_temp1.33.npy'))
            self.train_3, _, _, _ = train_test_split(self.train_3, self.train_transcript_2, test_size=0.05, random_state=100)

            self.train_4 = self.expand(np.load(params.data_dir + 'feats40dim_train_temp0.67.npy'))
            self.train_4, _, _, _ = train_test_split(self.train_4, self.train_transcript_2, test_size=0.05, random_state=100)
